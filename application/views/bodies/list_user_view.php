<div class="container-fluid paper-back">
 	<div class="row">
		<h1 id="ent_usr"><?=$this->lang->line('s_usr_mh') ?></h1>
    	<div id="user-map" style="width:100%;height:150px"></div>
  	</div>
    <div class="row">
      <div class="">
			<div id="list_usr">
				...
				<?php foreach ($lista as $key ) :?>
                 <tr
                  <?php if($key->revisado==1) :?>
                   class="success"
               <?php elseif($key->revisado==2): ?>
                    class="danger"
                   <?php else:?>
                   class="warning"
                <?php endif; ?>
                 ><td><span class ="badge"><?=$key->visto ?></span>  <?=strtoupper(html_entity_decode($key->titulo))?></td>
            <td>
                 <button onclick="ver_art(<?=$key->id_registro?>);return false;" class="btn btn-info btn-xs fa fa-eye"></span></button>
                 <?php if($key->revisado==1 || $key->revisado==2): ?>
             	 <button class="btn btn-default btn-xs fa fa-pencil"></button>
             	 <button  class="btn btn-success btn-xs fa fa-check"></button>
             	 <button onclick="errase_h(<?=$key->id_registro?>);return false;" class="btn btn-danger btn-xs fa fa-trash-o"></button>
             <?php else: ?>
                 <a style="color:green" href="<?=site_url('list/edicio/historia').'/'.$key->id_registro?>"> <button class="btn btn-primary btn-xs fa fa-pencil"></button></a>
                 <button  data-item="<?=$key->id_registro?>" thref="<?=site_url('list/edicio/publicar').'/'.$key->id_registro?>" class="btn btn-success btn-xs fa fa-paper-plane send-btn"></button>
                 <button class="btn btn-default btn-xs fa fa-trash-o"></button>
         <?php endif; ?>
		...
		<?php else: ?>
		<p><?=$this->lang->line('s_usr_ne') ?></p>
			<?php endif; ?>
			</div>
		</div>
		<button class="btn btn-info btn-xs" id="new-h" ><span class="glyphicon glyphicon-plus"></span> Relato nuevo</button>
		</div>
</div>
<!-- Modal -->
<div class="modal fade" id="new-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
